...
- 抽出品質検証（step 4.5）は extraction_quality.json が存在すればスキップ
- 特徴量CSVは1画像ごとに追記されるため抽出も途中から再開される
- 合格後の特徴量選択（step 7.5）はLLM不要かつ決定的なので、中断時は再実行でやり直す
"""
...
def run(
    ...
    while True:
        ...
        # step 4.5: 新規/変更特徴量の抽出品質検証（trainサンプル・再開対応）
        extraction_quality = None
        if cfg.verification_enabled:
            extraction_quality = run_verification(
                schema,
                prev_schema,
                splits["train"]["filename"].tolist(),
                images_dir,
                cfg,
                iteration,
                out_json=results_dir / f"iter{iteration}" / "extraction_quality.json",
                prev_json=results_dir / f"iter{iteration - 1}" / "extraction_quality.json",
                vlm_fn=vlm_fn,
            )
        ...
        if passed:
            # step 7.5: 特徴量選択（後方消去）。test は使わない
            schema_for_test = schema
            model_for_test = result["_model"]
            selection = None
            if cfg.feature_selection_enabled:
                selection = select_features(
                    ...
                    splits,
                    result,
                    out_json=results_dir / "feature_selection.json",
                    schema_path=schemas_dir / f"metadata_v{iteration}_selected.json",
                    model_path=models_dir / f"model_v{iteration}_selected.txt",
                )
                schema_for_test = selection["schema"]
                model_for_test = selection["result"]["_model"]

            # step 8: test で最終評価（1回のみ）
            test_result = evaluate_on_test(
                cfg, schema_for_test, features_df, splits["test"], model_for_test
            )
            ...
            if selection is not None:
                state["feature_selection"] = {
                    "n_features_before": selection["n_features_before"],
                    "n_features_after": selection["n_features_after"],
                    "removed": selection["removed"],
                    "val_score_before": selection["baseline_val_score"],
                    "val_score_after": selection["final_val_score"],
                }
